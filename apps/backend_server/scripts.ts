...
export async function Scraper({ api_key, url, country, product }: ScraperProps): Promise<ScraperResponse | null> {
...
function splitIntoBatches({ array, batchSize = 3 }: { array: string[]; batchSize?: number }): string[][] {
  const batches = [];
